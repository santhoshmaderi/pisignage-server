...
//         if (!data) {
//             return rest.sendError(res, 'Gcal file read error');
//         }
//         
//         // Parse JSON
//         const calData = JSON.parse(data);
//         
//         // Fetch calendar list from Google (need to promisify gcal.index)
//         const gcal = await import('./gcal.js');
//         const list = await gcal.index(calData);
//         
//         // Transform list - extract only id and summary
//         const transformedList = list.items.map(item => ({
//             id: item.id,
//             summary: item.summary
//         }));
//         
//         // Find selected calendar
//         const selectedCal = list.items.find(item => item.id === calData.selectedEmail);
//         const selectedSummary = selectedCal ? selectedCal.summary : null;
//         
//         // Send response
//         return rest.sendSuccess(res, 'Sending calendar details', {
//             profile: calData.profile,
